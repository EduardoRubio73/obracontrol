-- Corrige violacao de fornecedores_user_id_fkey no envio publico de propostas.
--
-- submit_public_proposta criava um fornecedor "publico" com
-- user_id = '00000000-0000-0000-0000-000000000000', mas fornecedores.user_id
-- tem FK para auth.users(id) e esse uuid nao existe la -> toda proposta nova
-- (empresa ainda nao cadastrada) falhava com "violates foreign key constraint
-- fornecedores_user_id_fkey".
--
-- Além de quebrado, o uuid fake também deixava o fornecedor invisível para o
-- dono da obra: a policy fornecedores_select exige user_id = auth.uid(), entao
-- mesmo se o insert funcionasse o nome do fornecedor sumiria do join em
-- propostas(fornecedores.nome).
--
-- Correcao: o fornecedor criado pelo formulario publico passa a ser
-- registrado com o user_id do dono da obra (dono da cotacao), igual a
-- qualquer fornecedor cadastrado manualmente por ele.

...
function useCatalogCrud(table: string) {
  const queryClient = useQueryClient();
  ...
  const add = useMutation({
    mutationFn: async ({ nome, descricao, extra }: { nome: string; descricao?: string; extra?: Record<string, unknown> }) => {
      const dup = (items ?? []).find((i) => (i.nome ?? "").toLowerCase().trim() === nome.toLowerCase().trim());
      if (dup) throw new Error(`Já existe um item com o nome "${nome}".`);
      const { error } = await supabase.from(table as any).insert({ nome, descricao: descricao || null, ativo: true, ...extra } as any);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    onError: (e: any) => {
      if (e.message?.includes("violates foreign key") || e.message?.includes("RESTRICT")) {
        ...
        toast.error("Acesso negado: apenas administradores podem editar o catálogo.");
      } else toast.error(e.message);
    },
  });
  ...
  const del = useMutation({
    ...
    onError: (e: any) => {
      if (e.message?.includes("violates foreign key") || e.message?.includes("RESTRICT") || e.code === "23503") {
        ...
        toast.error("Acesso negado: apenas administradores podem excluir do catálogo.");
      } else toast.error(e.message);
    },
  });
...
/* Catalog CRUD body (nome + descricao + ativo) */
function CatalogBody({ table, label }: { table: string; label: string }) {
  const { items, isLoading, add, update, del } = useCatalogCrud(table);
  const [novoNome, setNovoNome] = useState("");
  const [novaDescricao, setNovaDescricao] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingNome, setEditingNome] = useState("");
  const [editingDescricao, setEditingDescricao] = useState("");
  const [editingAtivo, setEditingAtivo] = useState(true);
  ...
  const startEdit = (item: CatalogItem) => {
    ...
    setEditingAtivo(item.ativo ?? true);
  };
  const cancelEdit = () => { setEditingId(null); setEditingNome(""); setEditingDescricao(""); setEditingAtivo(true); };
  const saveEdit = () => {
    if (!editingNome.trim() || !editingId) return;
    update.mutate({ id: editingId, nome: editingNome.trim(), descricao: editingDescricao.trim(), extra: { ativo: editingAtivo } }, { onSuccess: cancelEdit });
  };
    ...
    <div className="space-y-3 pt-3">
      ...
      <div className="space-y-1">
        ...
        {!isLoading && !items?.length && (
          <p className="text-muted-foreground text-sm text-center py-4">Nenhum(a) {label} no catálogo.</p>
        )}
        {filtrados?.map((item) => (
            ...
            {editingId === item.id ? (
              <div className="flex flex-col gap-2 flex-1 mr-2">
                ...
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={editingAtivo} onChange={(e) => setEditingAtivo(e.target.checked)} />
                  <span>Ativo</span>
                </label>
              ...
              <>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {!item.ativo && <Badge variant="outline" className="text-xs">Inativo</Badge>}
                  </div>
...
function CatalogServicosBody() {
  const { items, isLoading, add, update, del } = useCatalogCrud("catalogo_servicos");
  const [novoNome, setNovoNome] = useState("");
  const [novaDescricao, setNovaDescricao] = useState("");
  const [novaPrioridade, setNovaPrioridade] = useState<number | null>(null);
  const [novoTempo, setNovoTempo] = useState<number | null>(null);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editingNome, setEditingNome] = useState("");
  const [editingDescricao, setEditingDescricao] = useState("");
  const [editingPrioridade, setEditingPrioridade] = useState<number | null>(null);
  const [editingTempo, setEditingTempo] = useState<number | null>(null);
  const [editingAtivo, setEditingAtivo] = useState(true);
  const [busca, setBusca] = useState("");
  ...
  const startEdit = (item: CatalogItem) => {
    ...
    setEditingTempo(item.tempo_medio_dias ?? null);
    setEditingAtivo(item.ativo ?? true);
  };
  const cancelEdit = () => {
    setEditingId(null);
  ...
  return (
    <div className="space-y-3 pt-3">
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Input
            ...
            onChange={(e) => setNovaPrioridade(e.target.value ? parseInt(e.target.value) : null)}
          />
          <Input
            type="number"
            placeholder="Tempo médio (dias)"
            value={novoTempo ?? ""}
            onChange={(e) => setNovoTempo(e.target.value ? parseInt(e.target.value) : null)}
          />
        </div>
        <Button onClick={handleAdd} disabled={!novoNome.trim() || dupName || add.isPending} className="w-full sm:w-auto">
          <Plus className="h-4 w-4 mr-1" /> Adicionar
      ...
      <div className="space-y-1">
        {isLoading && <p className="text-muted-foreground text-sm text-center py-4">Carregando...</p>}
        {!isLoading && !items?.length && (
          <p className="text-muted-foreground text-sm text-center py-4">Nenhum serviço no catálogo.</p>
        )}
        {filtrados?.map((item) => (
          <div key={item.id} className="flex items-start justify-between py-2 px-2 border-b last:border-0 hover:bg-muted/50 rounded-lg transition-colors">
            {editingId === item.id ? (
              <div className="flex flex-col gap-2 flex-1 mr-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Input
                    type="number"
                    placeholder="Prioridade"
                    value={editingPrioridade ?? ""}
                    onChange={(e) => setEditingPrioridade(e.target.value ? parseInt(e.target.value) : null)}
                  />
                  <Input
                    type="number"
                    placeholder="Tempo médio (dias)"
                    value={editingTempo ?? ""}
                    onChange={(e) => setEditingTempo(e.target.value ? parseInt(e.target.value) : null)}
                  />
                </div>
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={editingAtivo} onChange={(e) => setEditingAtivo(e.target.checked)} />
              ...
              <>
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 mt-1 flex-wrap">
                    {item.prioridade !== null && <Badge variant="secondary" className="text-xs">P{item.prioridade}</Badge>}
                    {item.tempo_medio_dias !== null && <Badge variant="secondary" className="text-xs">{item.tempo_medio_dias}d</Badge>}
                  </div>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <Button variant="ghost" size="icon" onClick={() => startEdit(item)} className="h-8 w-8">
...
function CatalogCollapsible({
  ...
  return (
    <CollapsibleCard title={title} icon={icon} tooltip={tooltip} count={items?.length}>
      <CatalogBody table={table} label={label} />
    </CollapsibleCard>
  );
}

function CatalogServicosCollapsible() {
  const { data: items } = useQuery({
    queryKey: ["catalogo_servicos"],
    queryFn: async () => {
      const { data } = await supabase.from("catalogo_servicos").select("id");
      return data ?? [];
    },
  });
  return (
    <CollapsibleCard
      title="Serviços"
      icon="🔧"
      tooltip="Serviços padrão do catálogo com prioridade e tempo médio estimado."
      count={items?.length}
    >
      <CatalogServicosBody />
    </CollapsibleCard>
      ...
      <Tabs defaultValue="materiais">
        <ScrollArea className="w-full">
          <TabsList className="inline-flex w-auto min-w-full">
            ...
            <TabsTrigger value="catalogo">🎨 Catálogo Mestre</TabsTrigger>
          </TabsList>
        ...
        <TabsContent value="catalogo" className="space-y-3 mt-4">
          <CatalogCollapsible
            table="catalogo_tipos_obra"
            label="tipo de obra"
            title="Tipos de Obra (Catálogo)"
            icon="🏗️"
            tooltip="Tipos de obra compartilhados entre todos os usuários. Apenas administradores podem editar."
          />
          <CatalogCollapsible
            table="catalogo_ambientes"
            label="ambiente"
            title="Ambientes (Catálogo)"
            icon="🏠"
